import"./hoisted.DF-rxbhH.js";const n=document.getElementById("search-input"),i=document.getElementById("search-container"),{posts:p,authors:d}=window.__BLOG_DATA__,g=e=>{const r=d.find(o=>o.id===e.authorId),t=e.translatorId?d.find(o=>o.id===e.translatorId):null,s=`
        <div class="absolute inset-0 bg-gradient-to-br from-primary/10 via-muted to-primary/5 flex flex-col items-center justify-center p-4 text-center border border-dashed border-primary/20">
          <span class="font-kufam text-lg font-bold text-foreground/50 leading-snug line-clamp-3">${e.title}</span>
        </div>`,l=e.thumbnail?`<img src="${e.thumbnail}" alt="${e.title}" class="relative z-10 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 bg-background" onerror="this.style.display='none';" loading="lazy" />`:"",c=s+l,a=r?`
        ...
        </span>`:"",f=(Array.isArray(e.category)?e.category:[]).map(o=>`<a href="/search?q=${o}" class="bg-primary/10 px-2 py-1 rounded-md hover:bg-primary hover:text-primary-foreground transition-colors font-sans">${o}</a>`).join("");return`
      <div class="group flex flex-col sm:flex-row gap-6 bg-card border border-border p-4 rounded-xl hover:shadow-md transition-all duration-300">
        ...
        <div class="flex flex-col flex-grow py-2">
          <div class="flex flex-wrap items-center gap-2 text-sm text-primary font-medium mb-2 font-vazirmatn">
            ${f}
          ...
          <p class="text-muted-foreground mb-6 line-clamp-2 leading-relaxed font-vazirmatn text-[15px]">${e.excerpt}</p>
          <div class="mt-auto flex items-center justify-between border-t border-border/50 pt-4 font-vazirmatn">
            <div class="flex flex-col gap-1 text-sm">${a}${h}</div>
      ...
      </div>`},m=()=>{const e=n.value.trim().toLowerCase();if(!e&&!i)return;const r=p.filter(t=>{const s=d.find(a=>a.id===t.authorId),l=t.translatorId?d.find(a=>a.id===t.translatorId):null,c=Array.isArray(t.category)?t.category:[];return t.title.toLowerCase().includes(e)||t.excerpt.toLowerCase().includes(e)||t.content.toLowerCase().includes(e)||c.some(a=>a.toLowerCase().includes(e))||s&&s.name.toLowerCase().includes(e)||l&&l.name.toLowerCase().includes(e)});if(!e){i.innerHTML="";return}r.length===0?i.innerHTML=`
        ...
        `},u=new URLSearchParams(window.location.search);u.has("q")&&(n.value=u.get("q")||"",m());n.addEventListener("input",()=>{const e=new URL(window.location.href);n.value?e.searchParams.set("q",n.value):e.searchParams.delete("q"),window.history.replaceState({},"",e),m()});
