import"./hoisted.DF-rxbhH.js";const n=document.getElementById("search-input"),i=document.getElementById("search-container"),{posts:f,authors:d}=window.__BLOG_DATA__,x=e=>{const r=d.find(o=>o.id===e.authorId),t=e.translatorId?d.find(o=>o.id===e.translatorId):null,s=e.thumbnail?`<img src="${e.thumbnail}" alt="${e.title}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />`:'<div class="w-full h-full flex items-center justify-center text-muted-foreground text-sm">بدون تصویر</div>',l=r?`
        ...
        </span>`:"",h=(Array.isArray(e.category)?e.category:[]).map(o=>`<a href="/search?q=${o}" class="bg-primary/10 px-2 py-1 rounded-md hover:bg-primary hover:text-primary-foreground transition-colors font-sans">${o}</a>`).join("");return`
      <div class="group flex flex-col sm:flex-row gap-6 bg-card border border-border p-4 rounded-xl hover:shadow-md transition-all duration-300">
        ...
        <div class="flex flex-col flex-grow py-2">
          ...
          <div class="mt-auto flex items-center justify-between border-t border-border/50 pt-4 font-vazirmatn">
            <div class="flex flex-col gap-1 text-sm">${l}${c}</div>
